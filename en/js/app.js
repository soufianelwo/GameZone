document.addEventListener('DOMContentLoaded', function () {
    ...
    // Build categories array: first "All games", then all options except the first (which is "All catégories")
    const categories = [{ name: 'All games', value: '' }];
    ...
    categories.forEach((cat, idx) => {
        ...
        a.href = cat.value === '' ? '/en/index.html' : `/en/${cat.value.toLowerCase().replace(' ', '') }.html`;
...
function renderGames() {
  ...
  pageGames.forEach((game, idx) => {
    ...
    card.addEventListener('click', () => {
        window.location.href = `/en/game/${slugifygamehome(game.title)}`;
    });
